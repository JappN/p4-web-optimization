...
var resizePizzas = function (size) {
	...
	// Changed querySelector to document.getElementById
	...
	var theWidth = document.querySelector("#randomPizzas").offsetWidth; //created var to hold this so width wouldn't constantly be called in function below. improves speed.
	// Returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size).
	function determineDx(elem, size) {
		var oldwidth = elem.offsetWidth;
		var windowwidth = theWidth;
	...
	/* Iterates through pizza elements on the page and changes their widths, as per https://www.udacity.com/course/viewer#!/c-ud860-nd/l-4147498575/e-4154208580/m-4240308553;
	Changed document.querySelectorAll to document.getElementsByClassName*/
	function changePizzaSizes(size) {
		var allContainer = document.getElementsByClassName(".randomPizzaContainer");
		var dx = determineDx(allContainer[0], size);
		var newWidth = (allContainer[0].offsetWidth + dx) + 'px';
...
//declares the pizzasDiv variable outside the loop, so the function only makes one DOM call.
...
//pulled document.body.scrollTop and var phase out of the loop and assigned to var theTop to improve speed
function updatePositions() {
	...
	var phaseArray = [];
	var theTop = (document.body.scrollTop / 1250);
	for (i = 0; i < 5; i++) {
		phaseArray.push(Math.sin(theTop + i % 5));
	}
...
// Declared the elem variable out of the loop.
